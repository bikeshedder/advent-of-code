...
impl Space {
    ...
    fn calc_index(&self, p: Point) -> usize {
        (self.size.0 * self.size.1 * p.0 + self.size.0 * p.1 + p.2) as usize
    }
    ...
    (-1..=1)
        .map(|z| {
            (-1..=1)
                .map(|y| {
                    (-1..=1)
                        .map(|x| space[(p.0 + z, p.1 + y, p.2 + x)] as usize)
                        .sum::<usize>()
                })
                .sum::<usize>()
        })
...
fn main() {
    let size = (SIZE * 2 + 1, SIZE * 2 + 1, SIZE * 2 + 1);
    ...
    for y in 0..input_height {
        for x in 0..input_width {
            space[(SIZE, y + input_origin.0, x + input_origin.1)] = input[y as usize][x as usize];
        }
    }
    for _ in 0..STEPS {
        step(&mut space);
